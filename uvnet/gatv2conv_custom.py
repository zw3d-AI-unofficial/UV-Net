...
class GATv2Conv(nn.Module):
    ...
    def __init__(
        ...
        self.fc_edge = nn.Linear(self._in_src_feats, out_feats * num_heads, bias=bias)
    ...
    def reset_parameters(self):
        ...
        if isinstance(self.fc_edge, nn.Linear):
    ...
    def merge_edge_features(self, edges):
        # 访问源节点特征
        source_feats = edges.src['el']
        # 访问目标节点特征
        dest_feats = edges.dst['er']

        edge_feats = edges.data["efeat"]

        return {'e': source_feats + dest_feats + edge_feats}

    def forward(self, graph, feat, e_feat, get_attention=False):
        r"""
        Description
        -----------
        ...
        h_edge = self.fc_edge(h_edge).view(
            -1, self._num_heads, self._out_feats
        )
        ...
        graph.edata['efeat'] = h_edge
        graph.apply_edges(self.merge_edge_features)
        e = self.leaky_relu(
            graph.edata.pop("e")
        )  # (num_src_edge, num_heads, out_dim)
        e = (
